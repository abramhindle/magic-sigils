...
let reader key sock fdi =
  try while true do 
	...
	let _ =  status "Reading a size" in
	...
	let _ =  status "Reading a message" in
...
let () =
  ...
  let sock = Unix.socket Unix.PF_INET Unix.SOCK_STREAM 0 in
    ...
    while true do
      Unix.listen sock 1;
      let child, parent = Unix.socketpair Unix.PF_UNIX Unix.SOCK_STREAM 0 in
	...
	if pid = 0 then begin
	  ...
	  0 -> client
      let thread = Thread.create server sock in
	Queue.add thread q
    done
